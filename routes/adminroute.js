...
router.post('/adminlogin', (req, res) => {
  ...
  db.query(sql, [req.body.email, req.body.password], (err, data) => {
    ...
    if (data.length > 0) {
      ...
      let updateTokenQuery = `UPDATE admin SET cookie = ? WHERE email = ?`;
...
router.post('/addbook', upload.single('image'), (req, res) => {
  let sql = `insert into book (name,author,semester,price,status,quantity,image) values (?)`;
  const value = [
    req.body.name,
    req.body.author,
    req.body.semester,
...
router.get('/adminprofile', (req, res) => {
  ...
  const sql = 'select * from admin where cookie = ?';
...
const sendMailToUser = async (userId, id, bookid) => {
  const sql1 = `SELECT m.reminder_sent, m.id,u.email FROM mail m join users u on m.userid=u.id WHERE m.id = ?`;
  db.query(sql1, [id], (err, data) => {
    if (err) {
      console.log(err);
    } else {
      ...
      transporter.sendMail(options, (err, info) => {
        if (err) {
          ...
          const sql2 = `UPDATE mail SET reminder_sent = 1 WHERE id = ?`;
